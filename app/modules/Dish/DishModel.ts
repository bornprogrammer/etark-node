...
interface IDishInstance extends Instance<IDish>, IDish {}
...
const DishModel = sequelize.define<IDishInstance, IDish>(
    'dishes',
    {
        id: {
            type: Sequelize.INTEGER,
        ...
        level: {type: Sequelize.STRING},
        points: {type: Sequelize.INTEGER},
        currentWeekPoints: {type: Sequelize.INTEGER, field: 'current_week_points'},
        lastWeekPoints: {type: Sequelize.INTEGER, field: 'last_week_points'},
        isAccountUpdated: {type: Sequelize.INTEGER, field: 'is_account_updated'},
        postType: {type: Sequelize.STRING, field: 'post_type'},
        bookmarks: {type: Sequelize.STRING},
        ...
        createdAt: {type: Sequelize.DATE, field: 'created_at'},
        updatedAt: {type: Sequelize.DATE, field: 'updated_at'},
       // add two keys here getCountOD
    },
);

// DishModel.associate = (models) => {
//     DishModel.belongsTo(models.UserModel);
// };

//  let associate = (model)=>{
//     DishModel.belongsTo(model.UserModel);
//     UserModel.hasMany(model.DishModel)
// }

// associate({DishModel,UserModel});

DishModel.belongsTo(UserModel, {foreignKey: 'buddy_id'});
DishModel.hasMany(CustomMenuDishes);

DishModel.hasMany(Order);
