    ...
    public extractOutAllComplaintDetails = async (complaint: Complaint[]) => {
        ...
        if (complaint) {
            ...
            complaint.forEach((complaintItem) => {
                ...
                let complaintInfo = { complain_id: complaintItem.id, order_no: "", complainStatus: complaintItem.status, complaintCreatedAt: DateHelper.convertUTCDateToLocal(complaintItem['createdAt']), userName: complaintItem.user.name, userEmail: complaintItem.user.email, userMobileNumber: complaintItem.user.mobile_number, grand_total: 0, payment_status: "", sub_total: 0, tax: 0, gateway_charge: 0, plan_name: "", plan_components_details: "", delivery_amount: 0, distance_meters: 0, delivery_status: "", service_center_status: "", sc_imei_number: "", device_front_image: "", device_back_image: "", phone_warranty: "", service_to_be_done: "", invoice_total_amount: 0, proforma_invoice_image: "", device_delivery_date: "", due_date: "", not_warranty_reason: "", sc_order_created_at: "", device_front_image1: "", device_back_image1: "", final_invoice_image: "", final_invoice_amount: 0, ready_to_dispatch_created_at: "", user_address: "" };
                ...
                if (complaintItem.userPlan.pickupDeliveryDetail) {
                    ...
                    if (ObjectHelper.isObjectNotEmpty(pickupDeliveryDetails.userAddress)) {
                        let userAddress = pickupDeliveryDetails.userAddress;
                        let address = `lat : ${userAddress['lat']} long : ${userAddress['lon']} addresss : ${userAddress['address']} zip : ${userAddress['zip_code']}`;
                        complaintInfo.user_address = address;
                    }
